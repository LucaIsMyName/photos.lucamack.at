...
const AllImagesMap = () => {
  ...
  return (
    <div className="w-full h-full md:h-screen relative overflow-hidden">
      <SeoHead
        title={`Foto Karte | ${CONFIG.meta.title}`}
        description={`Eine interaktive Karte mit allen Fotos, die Geodaten enthalten.`}
        imageUrl={getImageUrl("all", galleries[0].images[0].filename.replaceAll(" ", "_"), 1440)}
      />

      <Legend
        galleryColors={galleryColors}
        legendToggleRef={legendToggleRef}
