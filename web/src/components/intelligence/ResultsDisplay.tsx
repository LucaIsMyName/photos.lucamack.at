...
const ResultsDisplay: React.FC<ResultsDisplayProps> = ({ images, message }) => {
  const { theme } = useTheme(); const formatDate = (dateString: string) => {
    ...
    <div className="space-y-4">
      {message && (
        <div className={`p-3 border ${theme === 'dark' ? '' : ''
          }`}>
        ...
        {images.map((image, index) => (
          <div
            ...
            className={`border overflow-hidden ${theme === 'dark' ? 'bg-black border-white' : 'bg-white border-black'
              }`}
          >
            ...
            <div className="p-3 space-y-2">
              ...
              <Link
                ...
                className={`text-xs underline ${theme === 'dark' ? 'text-red-400 hover:text-red-300' : 'text-red-600 hover:text-red-700'
                  }`}
              >
                {image.galleryTitle}
              </Link>              <div className="flex flex-wrap gap-2 text-xs">
                {image.createDate && (
              ...
              </div>              {image.aiAnalysis && image.aiAnalysis.labels.length > 0 && (
                <div className="space-y-1">
                  <div className={`flex items-center gap-1 text-xs ${theme === 'dark' ? 'text-gray-300' : 'text-gray-600'
                    }`}>
                    <Eye size={12} />
                    AI erkannt:
                  </div>
                  <div className="flex flex-wrap gap-1">
                    {image.aiAnalysis.labels.slice(0, 3).map((label, idx) => (
                      <span
                        key={idx}
                        className={`px-2 py-1 text-xs border ${theme === 'dark'
