...
async function generateGalleries() {
  ...
  const galleryPromises = directories.map(async (name) => {
    ...
    const imageFilesRaw = fs.readdirSync(galleryDir)
      .filter(file => /\.(jpe?g|png|gif|webp|heic)$/i.test(file))
      .map(file => file.replace(/\.heic$/i, '.jpg'));
    const imageFiles = [...new Set(imageFilesRaw)];
    ...
    return { name, slug: name, title, description, images: imageFiles };
  });
